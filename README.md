...
* `/setupcontrolpanel <type> [channel]` - Spawn the interactive Member or Admin Control Panel. The Admin Control Panel includes **Copy Raiders**, **Raffle Raider**, **WL Purchases**, and **Delete All Data** buttons.
...
* **WL Purchases** (Admin Control Panel Button) - Instantly retrieves a list of all active whitelist role purchases from the database, detailing who bought the role, which item they bought, the purchase timestamp, and the exact expiration date/time (with local timezone conversion and remaining duration).
...
* **Secure Database Reset**: Added a **Delete All Data** confirmation workflow for administrators both in the Next.js Dashboard (`/api/admin/delete-all-data`) and as an interactive button on the Discord Admin Control Panel, protected by a dual-stage warning and a typing confirmation requirement (`I want to Fuck Chess Dao Data Base`).
* **Chronological Marketplace Sorting**: Updated the Discord bot and Next.js Web Dashboard to list marketplace items chronologically by creation date (`createdAt: 1`) instead of alphabetically, ensuring that items are displayed in the exact order they are listed/added.
* **Live Marketplace Role Mention**: Updated the live marketplace channel embed message to mention the target ping role (`1478383117776715970`) outside the embed border (above the embed) for permanent visibility and ping notifications.
